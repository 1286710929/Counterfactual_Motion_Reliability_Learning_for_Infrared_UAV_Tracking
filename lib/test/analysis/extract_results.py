...
def calc_antiuav_state_accuracy(pred_bb, anno_bb, target_visible):
    ...
    if pred_bb.shape[1] < 4:
        pad = torch.zeros((pred_bb.shape[0], 4 - pred_bb.shape[1])).type_as(pred_bb)
        pred_bb = torch.cat((pred_bb, pad), dim=1)
    elif pred_bb.shape[1] > 4:
        pred_bb = pred_bb[:, :4]
    pred_bb[0, :] = anno_bb[0, :]
    ...
    pred_absent = ((pred_bb[:, 2:] <= 0.0).any(dim=1)) | ((pred_bb.abs().sum(dim=1) <= 0.0))
    state_score = torch.zeros(pred_bb.shape[0], dtype=torch.float64, device=pred_bb.device)
    state_score[target_visible] = iou[target_visible]
    state_score[~target_visible] = pred_absent[~target_visible].double()
    return state_score.mean()
...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    state_accuracy = torch.full((len(dataset), len(trackers)), -1.0, dtype=torch.float64)
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            if report_name in ["antiuav_rgbt_test"]:
                report_name = "antiuav_rgbt"
            ...
            if seq.dataset in ["antiuav410", "antiuav_rgbt"] and target_visible is not None:
                state_accuracy[seq_id, trk_id] = calc_antiuav_state_accuracy(pred_bb, anno_bb, target_visible)
    ...
    eval_data = {'sequences': seq_names, 
                 ...
                 'state_accuracy': state_accuracy.tolist(),
