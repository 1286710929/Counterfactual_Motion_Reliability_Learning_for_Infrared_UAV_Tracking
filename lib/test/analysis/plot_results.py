...
def merge_multiple_runs(eval_data):
    ...
    state_accuracy = torch.tensor(eval_data['state_accuracy']) if 'state_accuracy' in eval_data else None
    ...
    for i in range(len(trackers)):
        ...
        if state_accuracy is not None:
            state_accuracy_merged.append(state_accuracy[:, match].mean(1))
    ...
    if state_accuracy is not None:
        state_accuracy_merged = torch.stack(state_accuracy_merged, dim=1)
    ...
    if state_accuracy is not None:
        eval_data['state_accuracy'] = state_accuracy_merged.tolist()
