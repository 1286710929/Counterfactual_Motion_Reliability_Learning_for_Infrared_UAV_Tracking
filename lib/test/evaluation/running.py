...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['trackingnet', 'got10k_val', 'got10k_test','lasot', 'tnl2k', 'lasot_extension_subset', 'otb', 'got10k','antiuav410', 'antiuav_rgbt']:
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    def _results_exist():
        ...
        if seq.object_ids is None:
            if seq.dataset in ['trackingnet', 'got10k_test', 'got10k_val','lasot', 'tnl2k', 'lasot_extension_subset', 'otb', 'antiuav410_test', 'antiuav410', 'antiuav_rgbt']:
                base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
                bbox_file = '{}.txt'.format(base_results_path)
            else:
...
def _run_sequence_from_args(args):
...
def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8):
    ...
    if mode == 'sequential':
        ...
        for params in tqdm(param_list, total=len(param_list), desc='Testing', unit='seq'):
            run_sequence(*params)
    elif mode == 'parallel':
        # param_list = [(seq, tracker_info, debug, num_gpus) for seq, tracker_info in product(dataset, trackers)]
        param_list = [(seq, tracker_info, debug, num_gpus) for tracker_info, seq in product(trackers, dataset)]
        with multiprocessing.Pool(processes=threads) as pool:
            list(tqdm(pool.imap_unordered(_run_sequence_from_args, param_list),
                      total=len(param_list), desc='Testing', unit='seq'))
